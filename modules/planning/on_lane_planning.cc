...
Status OnLanePlanning::Init(const PlanningConfig& config) {//在初始化中，就确定使用哪个planner
  ...
  planner_dispatcher_->Init();//后面会使用到planner选择，因此初始化中也对选择器进行初始化
  ...
  // dispatch planner
  planner_ = planner_dispatcher_->DispatchPlanner(config_, injector_);//确定使用哪个planner
  if (!planner_) {
    return Status(
...
//注意：Plan（）中ptr_trajectory_pb还没有被所规划的填充轨迹点，frame_中的reference_line_info 才存放了，后面会利用frame_对ptr_trajectory_pb的trajectory_point_指针所指向的地址中
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {//如果在open_space
    ...
    last_publishable_trajectory_.reset(new PublishableTrajectory(
        current_time_stamp, best_ref_info->trajectory()));//利用初始构造，从best_ref_info成员discretized_trajectory_中获取所规划的轨迹，利用assign(trajectory.trajectory_point().begin(),trajectory.trajectory_point().end())函数拷贝到
    ...
    last_publishable_trajectory_->PopulateTrajectoryProtobuf(ptr_trajectory_pb);//这一步很重要，收尾，将所规划的轨迹点数据拷贝至trajectory_pb的trajectory_point_指针所指向的地址中
