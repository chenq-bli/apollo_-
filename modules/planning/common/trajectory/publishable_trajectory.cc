...
void PublishableTrajectory::PopulateTrajectoryProtobuf(
  ...
  trajectory_pb->mutable_trajectory_point()->CopyFrom({begin(), end()});//将所规划的轨迹点数据拷贝至trajectory_pb的trajectory_point_指针所指向的地址中，这里的{begin(), end()}是last_publishable_trajectory_作用域下的
  if (!empty()) {
