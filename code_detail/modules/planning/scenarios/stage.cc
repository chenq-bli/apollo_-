...
Stage::Stage(const ScenarioConfig::StageConfig& config,
  ...
  injector->planning_context()
      ...
      ->mutable_scenario()
      ->set_stage_type(stage_type()); //设置stage类型
  ...
  for (int i = 0; i < config_.task_type_size(); ++i) { //对该stage下的任务进行遍历，对task_list_填充
    ...
    auto iter = tasks_.find(task_type); // 获取first为task_type名字的map迭代器，tasks_的first为任务名字，second为Task的智能指针，该在哪指针指向实例化的任务对象
    if (iter == tasks_.end()) { //如果没有找到task_type，会返回tasks_.end()
      auto ptr = TaskFactory::CreateTask(*config_map[task_type], injector_);
      task_list_.push_back(ptr.get());
      tasks_[task_type] = std::move(ptr);
    } else {
      task_list_.push_back(iter->second.get());//get获裸指针，task_list_存放的不是智能指针
    }
  }
}
