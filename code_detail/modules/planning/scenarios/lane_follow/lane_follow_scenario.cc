...
std::unique_ptr<Stage> LaneFollowScenario::CreateStage(
  ...
  //此处，stage_config.stage_type()必定是LANE_FOLLOW_DEFAULT_STAGE
  return std::unique_ptr<Stage>(new LaneFollowStage(stage_config, injector));//创建LaneFollowStage，对子类和基类进行构造
}
