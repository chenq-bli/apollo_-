...
OSQPData* PiecewiseJerkProblem::FormulateProblem() {
  ...
  //CalculateKernel在PiecewiseJerkProblem中是纯虚函数，没有实现，在PiecewiseJerkProblem子类中进行了重写，因此类似与多态
  //CalculateKernel具体是调用哪个子类的，需要看当前是在哪个作用域
  ...
  size_t kernel_dim = 3 * num_of_knots_;//控制变量个数
  size_t num_affine_constraint = lower_bounds.size();//A矩阵行数，约束的个数
...
bool PiecewiseJerkProblem::Optimize(const int max_iter) {
  //这部分path plan 和speed plan 是共用的，但FormulateProblem中CalculateKernel，和CalculateAffineConstraint会对两者分别构造。
  OSQPData* data = FormulateProblem();//该函数是重点，主要包含了目标函数与响应约束的建立，以及目标函数的offset补偿。

  OSQPSettings* settings = SolverDefaultSettings();//求解器的默认设置
  settings->max_iter = max_iter;//设置最大迭代次数

  OSQPWorkspace* osqp_work = nullptr;
  osqp_work = osqp_setup(data, settings);//建立一个标准的求解问题，并用指针指向
  // osqp_setup(&osqp_work, data, settings);

  osqp_solve(osqp_work);//调用osqp来对该问题求解。
  ...
  // extract primal results，获取计算结果
  //此函数是piecewise_jerk_problem的成员函数，x_、dx_、ddx_为成员变量，可直接使用
  ...
  for (size_t i = 0; i < num_of_knots_; ++i) {
    ...
    dx_.at(i) = osqp_work->solution->x[i + num_of_knots_] / scale_factor_[1];//i + num_of_knots_是为了获取dx部分
    ddx_.at(i) =
        osqp_work->solution->x[i + 2 * num_of_knots_] / scale_factor_[2];//i + 2*num_of_knots_是为了获取ddx部分
  }
...
void PiecewiseJerkProblem::CalculateAffineConstraint(
  ...
  //这里可以看出有哪些约束，num_of_variables个不等式约束，3 * (n - 1)个连续性的等式约束，3个初始化状态的等式约束
  ...
  std::vector<std::vector<std::pair<c_int, c_float>>> variables(
      num_of_variables); //num_of_variables是变量个数共3n个
  

  int constraint_index = 0;
  // set x, x', x'' bounds
  for (int i = 0; i < num_of_variables; ++i) {//variables[0]-variables[n-1]为x
    if (i < n) {
      variables[i].emplace_back(constraint_index, 1.0);
      lower_bounds->at(constraint_index) =
          x_bounds_[i].first * scale_factor_[0];//x_bounds_为在PiecewiseJerkProblem对象内部的成员变量，前面配置的时候对其填充了
      upper_bounds->at(constraint_index) =
          x_bounds_[i].second * scale_factor_[0];
    } else if (i < 2 * n) { //variables[n]-variables[2n-1]为x'
      variables[i].emplace_back(constraint_index, 1.0);
    ...
    } else {//variables[2n]-variables[3n-1]为 x''
      variables[i].emplace_back(constraint_index, 1.0);
      lower_bounds->at(constraint_index) =
  ...
  for (int i = 0; i + 1 < n; ++i) {//连续性约束2
    variables[n + i].emplace_back(constraint_index, -1.0 * scale_factor_[2]);
    //理解到了，variables[k]相当于是对应第k个变量，这里的操作是在合并同类项
    //variables[i]相当于变量x(i)，variables[n + i] 相当于变量x(i)'，variables[2*n + i] 相当于变量x(i)'',注意倒数阶数变化
    //variables[i+1]相当于变量x(i+1)，variables[n + i+1] 相当于变量x(i+1)'，variables[2*n + i+1] 相当于变量x(i+1)'' 
    ...
    variables[2 * n + i + 1].emplace_back(constraint_index,
                                          -0.5 * delta_s_ * scale_factor_[1]);
    lower_bounds->at(constraint_index) = 0.0;//转化后为等式约束
  ...
  for (int i = 0; i + 1 < n; ++i) {//连续性约束3
    ...
    lower_bounds->at(constraint_index) = 0.0;//转化后等式约束
    upper_bounds->at(constraint_index) = 0.0;
    ++constraint_index;
  }
