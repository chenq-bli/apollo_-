...
void PiecewiseJerkPathProblem::CalculateKernel(std::vector<c_float>* P_data,
  ...
  //weight_end_state_[0]的由来，是末态和参考末态一致的代价，所以惩罚 (x - x_end_ref) ^ 2 和 (x '- dx_end_ref) ^ 2 和 (x '' - ddx_end_ref) ^ 2
  ...
  for (int i = 0; i < n - 1; ++i) {//0 - n-2
    columns[2 * n + i].emplace_back(2 * n + i + 1,
                                    (-2.0 * weight_dddx_ / delta_s_square) /
                                        (scale_factor_[2] * scale_factor_[2]));
    ++value_index;
  }//由于需要先转化为对称矩阵，然后内核矩阵记录上三角矩阵，所以这里的2.0不知道该不该有了。
  ...
  for (int i = 0; i < num_of_variables; ++i) {//对每个变量遍历
    ...
    for (const auto& row_data_pair : columns[i]) {//对每个变量对应的上面填充的vector进行遍历 
      P_data->push_back(row_data_pair.second * 2.0);//因为is set as (1/2) * x' * P * x，所以每个元素要*2
...
void PiecewiseJerkPathProblem::CalculateOffset(std::vector<c_float>* q) {
  ...
  if (has_x_ref_) {
    for (int i = 0; i < n; ++i) {//0 - n-1，只有前面n个，这验证了我的猜想：没有考虑(s '- dx_ref)^2代价项
      q->at(i) += -2.0 * weight_x_ref_vec_.at(i) * x_ref_[i] / scale_factor_[0];
    }
  }

  if (has_end_state_ref_) {//末态和参考末态一致代价产生的一次项
    q->at(n - 1) +=
