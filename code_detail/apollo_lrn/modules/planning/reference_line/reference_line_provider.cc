...
void ReferenceLineProvider::GenerateThread() {
  while (!is_stop_) {
    ...
    UpdateReferenceLine(reference_lines, segments);//将reference_lines写入reference_lines_
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  if (is_new_routing || !FLAGS_enable_reference_line_stitching) {
    for (auto iter = segments->begin(); iter != segments->end();) {
      reference_lines->emplace_back();//往后面腾出一个空间，来放下一个参考线
      if (!SmoothRouteSegment(*iter, &reference_lines->back())) { //平滑处理，输入参数为
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
  ...
  AnchorPoint anchor;
  anchor.longitudinal_bound = smoother_config_.longitudinal_boundary_bound();//设置一些约束条件
...
bool ReferenceLineProvider::SmoothRouteSegment(const RouteSegments &segments,
  ...
  return SmoothReferenceLine(ReferenceLine(path), reference_line);//具体的平滑源码
}
...
bool ReferenceLineProvider::SmoothReferenceLine(
  ...
  // generate anchor points:
  std::vector<AnchorPoint> anchor_points;
  GetAnchorPoints(raw_reference_line, &anchor_points);//去除一些不满足约束的点，获取需要平滑处理的路径点
  smoother_->SetAnchorPoints(anchor_points);//将anchor_points放入到smoother_对象里面
  if (!smoother_->Smooth(raw_reference_line, reference_line)) {//开始平滑，结果放在reference_line
