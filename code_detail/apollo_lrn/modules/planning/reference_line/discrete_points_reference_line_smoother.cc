...
bool DiscretePointsReferenceLineSmoother::Smooth(
  ...
  for (const auto& anchor_point : anchor_points_) {//遍历所有中点，将点和约束分别取出来
  ...
  anchorpoints_lateralbound.back() = 0.0;//起点和终点是硬约束

  NormalizePoints(&raw_point2d);//相对化处理

  bool status = false;

  const auto& smoothing_method = config_.discrete_points().smoothing_method();
  std::vector<std::pair<double, double>> smoothed_point2d;
  switch (smoothing_method) {//根据配置文件，选择使用哪个算法，这两种也就是计算平滑部分代价计算方式不一样
    ...
    case DiscretePointsSmootherConfig::FEM_POS_DEVIATION_SMOOTHING:
      status = FemPosSmooth(raw_point2d, anchorpoints_lateralbound,
                            &smoothed_point2d);//根据二维点，约束进行平滑，结果放在smoothed_point2d对象地址
      break;
    default:
  ...
  DeNormalizePoints(&smoothed_point2d);//逆处理

  std::vector<ReferencePoint> ref_points;
  GenerateRefPointProfile(raw_reference_line, smoothed_point2d, &ref_points);//利用平滑的smoothed_point2d，完善ref_points
  ...
  *smoothed_reference_line = ReferenceLine(ref_points);//构造一个ReferenceLine对象
  return true;
}
...
bool DiscretePointsReferenceLineSmoother::FemPosSmooth(
  ...
  const double box_ratio = 1.0 / std::sqrt(2.0);
  for (auto& bound : box_bounds) {
    bound *= box_ratio;//裕度收缩
  }

  std::vector<double> opt_x;
  std::vector<double> opt_y;
  bool status = smoother.Solve(raw_point2d, box_bounds, &opt_x, &opt_y);//求解器求解
