...
bool ScenarioManager::Init(const PlanningConfig& planning_config) {
  ...
  RegisterScenarios();//先注册
  default_scenario_type_ = ScenarioType::LANE_FOLLOW;
  current_scenario_ = CreateScenario(default_scenario_type_);//创建默认场景
  return true;
}
...
std::unique_ptr<Scenario> ScenarioManager::CreateScenario(
  ...
  switch (scenario_type) { //选择需要创建的场景，不一定是默认的
    ...
    case ScenarioType::LANE_FOLLOW:
      ptr.reset(new lane_follow::LaneFollowScenario(
          config_map_[scenario_type], &scenario_context_, injector_)); //删除之前关联的普通指针，再利用匿名对象实例化新目标场景的指针，并关联智能指针prt。
      break;
  ...
  if (ptr != nullptr) {
    ptr->Init(); //上面根据不同的场景，更新了智能指针，这里利用多态，分别调用各自场景的init().
  }
  return ptr;
}
...
void ScenarioManager::ScenarioDispatch(const Frame& frame) {
  ...
  if ((planning_config_.learning_mode() == PlanningConfig::E2E ||
    ...
    scenario_type = ScenarioDispatchLearning(); //设置scenario_type为LEARNING_MODEL_SAMPLE，通过强化学习获得场景类型
  } else {
  ...
  if (current_scenario_->scenario_type() != scenario_type) {
    current_scenario_ = CreateScenario(scenario_type); //只有场景变，才创建scenario_type类型的场景
  }
}
