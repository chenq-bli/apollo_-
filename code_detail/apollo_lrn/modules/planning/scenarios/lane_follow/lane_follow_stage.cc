...
LaneFollowStage::LaneFollowStage(
    ...
    : Stage(config, injector) {}//利用参数列表初始化Stage基类对象，基类继承下来的变量和函数，子类可用
...
//遍历所有的参考线，直到找到可以用来规划的参考线后退出
  ...
  //如果出现规划成功，而且未被舍去，则找到所规划轨迹，跳出循环
    ...
    auto cur_status =
        PlanOnReferenceLine(planning_start_point, frame, &reference_line_info);//传入的reference_line_info是不同的

    if (cur_status.ok()) {
      if (reference_line_info.IsChangeLanePath()) {//如果规划成功并且可变道
        ADEBUG << "reference line is lane change ref.";
        ADEBUG << "FLAGS_enable_smarter_lane_change: "
               << FLAGS_enable_smarter_lane_change;
        if (reference_line_info.Cost() < kStraightForwardLineCost && //规划成功后还需要与直行的cost比较，如果代价值更低则成功。
        ...
        } else {//规划成功后还需要与直行的cost比较，如果代价值更高，则舍去。
          LaneChangeDecider::UpdatePreparationDistance(
...
Status LaneFollowStage::PlanOnReferenceLine(
  ...
  auto ret = Status::OK();
  for (auto* task : task_list_) { //在决策场景的时候会创建lane_following_stage，同时对基类进行构造，task_list_在基类构造时被填充
    const double start_timestamp = Clock::NowInSeconds();

    ret = task->Execute(frame, reference_line_info);//由于多态的存在，Execute（）对应不同task内的函数Execute（）。
  ...
  if (!reference_line_info->CombinePathAndSpeedProfile(
          ...
          planning_start_point.path_point().s(), &trajectory)) {//调用reference_line_info成员函数，将路径规划和速度规划的结果整合到trajectory，每个reference_line_info只会获得一个trajectory
  ...
  reference_line_info->SetTrajectory(trajectory);//将trajectory放入reference_line_info成员discretized_trajectory_
  reference_line_info->SetDrivable(true);
  return Status::OK();
}
