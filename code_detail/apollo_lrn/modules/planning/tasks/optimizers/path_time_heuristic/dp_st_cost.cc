...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  if (FLAGS_use_st_drivable_boundary) {//如果使用可行驶边界（st_drivable_boundary）
    // TODO(Jiancheng): move to configs

    static constexpr double boundary_resolution = 0.1;//boundary_resolution表示可行驶边界的分辨率
    int index = static_cast<int>(t / boundary_resolution);//获取当前时间（t）所对应边界的索引
    const double lower_bound =
        st_drivable_boundary_.st_boundary(index).s_lower();
    const double upper_bound =
        st_drivable_boundary_.st_boundary(index).s_upper();//获取当前边界的上下限
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    if (boundary.min_s() > FLAGS_speed_lon_decision_horizon) {//障碍物超过了决策窗口范围，则后面的障碍物不遍历了
      continue;
    }
    if (t < boundary.min_t() || t > boundary.max_t()) {//如果当前时间不在障碍物的ST边界范围内，则跳过继续执行下一个障碍物。
      continue;
    }
    if (boundary.IsPointInBoundary(st_graph_point.point())) {//判断当前点是否在障碍物的ST边界内，如果是，则返回一个无穷大的代价，表示不可达。
      return kInf;
    }
    double s_upper = 0.0;
    double s_lower = 0.0;

    //如果当前时间在障碍物的ST边界范围内,且障碍物没有超过决策窗口范围

    int boundary_index = boundary_map_[boundary.id()];
    //如果之前没有计算过这个边界在当前时间下的s上下限
    if (boundary_cost_[boundary_index][st_graph_point.index_t()].first < 0.0) {
      boundary.GetBoundarySRange(t, &s_upper, &s_lower);//计算
      boundary_cost_[boundary_index][st_graph_point.index_t()] =
          std::make_pair(s_upper, s_lower);//填充
    } else {
    ...
    // ，但不在s上限和下限之间，则需要计算代价。
    // 根据位置和障碍物的距离，计算代价，并加入到总代价cost中。
    if (s < s_lower) {
      const double follow_distance_s = config_.safe_distance();
      if (s + follow_distance_s < s_lower) {//如果很安全
        continue;
      } else {
        auto s_diff = follow_distance_s - s_lower + s;//计算偏差
        cost += config_.obstacle_weight() * config_.default_obstacle_cost() *
                s_diff * s_diff;//距离障碍物越近，代价值越大
      }
    } else if (s > s_upper) {
  ...
  return cost * unit_t_;//unit_t_ 单个网格时间t的大小
}
