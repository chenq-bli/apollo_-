...
common::Status PiecewiseJerkPathOptimizer::Process(
  ...
  // skip piecewise_jerk_path_optimizer if reused path 道路复用跳过
  ...
  const auto init_frenet_state =
      reference_line.ToFrenetFrame(planning_start_point);//初始点转化到frent坐标系，包括s和l一级他们的速度、加速度
  ...
  const auto& config = reference_line_info_->IsChangeLanePath()
                           ...
                           : config_.piecewise_jerk_path_optimizer_config()
                                 .default_path_config();//根据场景选择参数文件
  ...
  std::array<double, 5> w = {
      ...
      config.dl_weight() *
          std::fmax(init_frenet_state.first[1] * init_frenet_state.first[1],
                    5.0),
      config.ddl_weight(), config.dddl_weight(), 0.0};//配置权重向量
//从上一任务获得的CandidatePathBoundaries赋值给path_boundaries，里面包括2条候补的，其中一条是fallback_path_bound，另外是pull_over_path_bound、lanechange_path_bound、regular_path_bound中的一个
  const auto& path_boundaries =
      reference_line_info_->GetCandidatePathBoundaries();
  ADEBUG << "There are " << path_boundaries.size() << " path boundaries.";//多少条候补的道路约束，
  const auto& reference_path_data = reference_line_info_->path_data();//一条参考线

  std::vector<PathData> candidate_path_data;
  for (const auto& path_boundary : path_boundaries) {//遍历每一条约束，第一条是fallback_path_bound，第二条是其他三种中任意的一种，并且regular_path_bound中有向左、向右或者不借到的情况，因此不止2组边界
    ...
    if (!FLAGS_enable_force_pull_over_open_space_parking_test) { 
      ...
      if (pull_over_status.has_position() &&
        ...
        reference_line.XYToSL(pull_over_status.position(), &pull_over_sl); //如果是靠边停车场景，将停车位置转化为SL坐标
    ...
    std::vector<double> path_reference_l(path_boundary_size, 0.0);//记录参考路径点的l
    bool is_valid_path_reference = false;
    size_t path_reference_size = reference_path_data.path_reference().size();//获取参考路径点的数量

    if (path_boundary.label().find("regular") != std::string::npos &&
        reference_path_data.is_valid_path_reference()) {
      ADEBUG << "path label is: " << path_boundary.label();
      // when path reference is ready
      for (size_t i = 0; i < path_reference_size; ++i) {//path_reference_size为采样点个数
        common::SLPoint path_reference_sl;//记录参考路径点的s和l
        reference_line.XYToSL(//xy坐标系转化为SL坐标系，并记录在path_reference_sl
      ...
      end_state[0] = path_reference_l.back();//终点状态的l位移
      path_data.set_is_optimized_towards_trajectory_reference(true);
      is_valid_path_reference = true;
    }

    //上面两个if是做判断，是靠边停车或是正常行驶，只有一个会被激活

    const auto& veh_param =
        common::VehicleConfigHelper::GetConfig().vehicle_param();
    const double lat_acc_bound =
        std::tan(veh_param.max_steer_angle() / veh_param.steer_ratio()) //veh_param.max_steer_angle()为方向盘转角，steer_ratio()为传动比
        / veh_param.wheel_base();
    std::vector<std::pair<double, double>> ddl_bounds;//l加速度约束
    ...
    // 是路径优化算法的具体实现，将路径优化问题转化为二次型规划问题，之后调用osqp库求解最优路径。
    //init_frenet_state为起始点状态，包括s和l方向的位移，速度和加速度；end_state为结束点状态；path_reference_l为该离散参考轨迹点的l位移，path_reference_size为离散点个数；path_boundary为边界约束
    //ddl_bounds为上面计算的ddl约束；w为权重向量；max_iter为最大迭代次数，opt_l、opt_dl、opt_ddl为待优化的求解变量
    bool res_opt = OptimizePath(
        ...
        &opt_ddl);
   //path_boundary.boundary()获取path_boundary类中的boundary_值，这个boundary_在path_bounds_decider任务中在emplace_back进入candidate_path_boundaries时的时候在容器内部进行了构造
    ...
    if (res_opt) {//如果规划成功
      ...
      path_data.SetReferenceLine(&reference_line);//参考path
      path_data.SetFrenetPath(std::move(frenet_frame_path));//规划的path，包括路径点由SL转化为XY，xy类型的路径点放在discretized_path_变量，sl类型的路径放在frenet_path_变量
      if (FLAGS_use_front_axe_center_in_path_planning) {
        auto discretized_path = DiscretizedPath(
            ConvertPathPointRefFromFrontAxeToRearAxe(path_data));
        path_data.SetDiscretizedPath(discretized_path);
      }
      path_data.set_path_label(path_boundary.label());//为path_data打上约束类型标签
      path_data.set_blocking_obstacle_id(path_boundary.blocking_obstacle_id());
      candidate_path_data.push_back(std::move(path_data));//上面有循环，将每组约束边界下所规划的路径加入candidate_path_data
    }
  }
  ...
  reference_line_info_->SetCandidatePathData(std::move(candidate_path_data));//将candidate_path_data放入到reference_line_info_的candidate_path_data_中
  //到此，不同的参考线下有不同的约束类型，然后不同的约束类型下有不同的优化路径。
  return Status::OK();
}
...
bool PiecewiseJerkPathOptimizer::OptimizePath(
    ...
    std::vector<double>* dx, std::vector<double>* ddx) {//别看这里是x，传入的是l，dl，ddl的vector
  // num of knots
  const size_t kNumKnots = lat_boundaries.size();//节点个数与约束点的个数一致
  //注意：直接创建的是子类对象，后面调用的一些成员函数是各子类的(包括继承的)成员函数，这就不需要使用多态了。
  //当然，也可以使用多态，这里父类指针指向创建的子类对象，后面调用重写函数时会根据子类对象的作用域进行分别调用。
  PiecewiseJerkPathProblem piecewise_jerk_problem(kNumKnots, delta_s,
                                                  init_state);//实例化一个piecewise_jerk_problem问题对象，进行初始化，后面有一系列成员函数进行一些参数的设置。

  // TODO(Hongyi): update end_state settings
  piecewise_jerk_problem.set_end_state_ref({1000.0, 0.0, 0.0}, end_state);//设置end_state_ref，end_state三纬分别是l,dl,ddl
  // pull over scenarios
  // Because path reference might also make the end_state != 0
  // we have to exclude this condition here
  if (end_state[0] != 0 && !is_valid_path_reference) {//如果path_reference无效，强行pull over
  ...
  if (is_valid_path_reference) {//如果path_reference有效
    ...
    piecewise_jerk_problem.set_x_ref(std::move(weight_x_ref_vec),
                                     path_reference_l_ref);//设将获取的权重和参考路径点的l放入到piecewise_jerk_problem对象
  }
  // for debug:here should use std::move//设置权重
  //将权重配置给该piecewise_jerk_problem类中weight_x_、weight_dx_和weight_ddx_,后面在成员函数中可直接使用
  ...
  piecewise_jerk_problem.set_scale_factor({1.0, 10.0, 100.0});//设置标记
...
//设置约束
  ...
  piecewise_jerk_problem.set_dx_bounds(-FLAGS_lateral_derivative_bound_default,
                                       FLAGS_lateral_derivative_bound_default);//根据传入参数设置dx_bounds
  piecewise_jerk_problem.set_ddx_bounds(ddl_bounds);//设置ddx_bounds，就是将ddl_bounds内部的数据转移到ddx_bounds_中

  // Estimate lat_acc and jerk boundary from vehicle_params 估计并设置加加速度约束
  const auto& veh_param =
      common::VehicleConfigHelper::GetConfig().vehicle_param();
  const double axis_distance = veh_param.wheel_base();
  const double max_yaw_rate =
      veh_param.max_steer_angle_rate() / veh_param.steer_ratio() / 2.0;//求解前轮转角角速度
  ...
  //估计加加速度约束，并添加
  //加加速度的约束是利用 最小转弯半径和轴距乘积 的倒数表示 即大约是1/(Rmin*L)，轴距是不变的，最小转弯半径倒数即对应最大的加加速度的约束

  bool success = piecewise_jerk_problem.Optimize(max_iter); //这里最重要，Optimize(max_iter)函数是qp问题求解的入口，包括调目标函数与响应约束的建立，以及调用osqp标准求解库求解。
...
//成功后，将x_，dx_，ddx_结果给到*x，*dx和*ddx
...
double PiecewiseJerkPathOptimizer::EstimateJerkBoundary(
  ...
  // max_yaw_rate最大车轮转角角速度、axis_distance为轴距、vehicle_speed（v）为ds
