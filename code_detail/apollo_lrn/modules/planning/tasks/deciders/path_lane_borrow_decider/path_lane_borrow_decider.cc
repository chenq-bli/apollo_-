...
Status PathLaneBorrowDecider::Process(
  ...
  // skip path_lane_borrow_decider if reused path 如果道路复用，则跳过路径规划,后面的也一直跳
  ...
  if (Decider::config_.path_lane_borrow_decider_config()
      ...
      IsNecessaryToBorrowLane(*frame, *reference_line_info)) { 
        //如果允许并且需要换道，则对 reference_line_info的借道属性填充
...
bool PathLaneBorrowDecider::IsNecessaryToBorrowLane(
  ...
  if (mutable_path_decider_status->is_in_path_lane_borrow_scenario()) {
    ...
    //如果当前车辆已经借用相邻车道 6 次及以上，则执行以下代码块。
    if (mutable_path_decider_status->able_to_use_self_lane_counter() >= 6) {
      ...
      // non-lane-borrowing.
      //将 is_in_path_lane_borrow_scenario 标志位设置为 false，表示车辆已经不再借用相邻车道。
    ...
    //如果当前车辆不在借用相邻车道模式，则执行以下代码块
    // If originally not borrowing neighbor lane:
    ADEBUG << "Blocking obstacle ID["//输出日志信息，记录当前车辆面对的障碍物的 ID
           << mutable_path_decider_status->front_static_obstacle_id() << "]";
    // ADC requirements check for lane-borrowing:
    if (!HasSingleReferenceLine(frame)) {//如果当前车道不是单行道，则返回 false。
      return false;
    }
    if (!IsWithinSidePassingSpeedADC(frame)) {//如果车速过快，不能进行侧向通行，则返回 false。
      return false;
    }

    // Obstacle condition check for lane-borrowing:
    if (!IsBlockingObstacleFarFromIntersection(reference_line_info)) {//如果障碍物距离路口太近，不能进行侧向通行，则返回 false。
      return false;
    }
    if (!IsLongTermBlockingObstacle()) {//如果障碍物不是长期阻挡障碍物，则返回 false。
      return false;
    }
    if (!IsBlockingObstacleWithinDestination(reference_line_info)) {//如果障碍物不在车辆的行驶目的地范围内，则返回 false,没必要变道。
      return false;
    }
    if (!IsSidePassableObstacle(reference_line_info)) {//如果障碍物不能通过侧向通行绕过，则返回 false。
      return false;
    }
    ...
    if (path_decider_status.decided_side_pass_direction().empty()) {
      ...
      //调用 CheckLaneBorrow 函数，判断左侧和右侧是否可借用相邻车道，并将结果分别存储在 left_borrowable 和 right_borrowable 中。
      CheckLaneBorrow(reference_line_info, &left_borrowable, &right_borrowable);
      if (!left_borrowable && !right_borrowable) {//如果不变道路
        mutable_path_decider_status->set_is_in_path_lane_borrow_scenario(false);//变道标准设置为不变道
        return false;
      } else {
        mutable_path_decider_status->set_is_in_path_lane_borrow_scenario(true);//变道标准设置为变道
        //这下面是两个if，如果左右两侧都可以变道则都会被填充为可变道
        if (left_borrowable) {//如果左侧变道
          mutable_path_decider_status->add_decided_side_pass_direction(
              PathDeciderStatus::LEFT_BORROW);
        }
        if (right_borrowable) {//如果右侧侧变道
          mutable_path_decider_status->add_decided_side_pass_direction(
  ...
  return mutable_path_decider_status->is_in_path_lane_borrow_scenario();//返回上面记录的结果
}
...
bool PathLaneBorrowDecider::IsBlockingObstacleWithinDestination(
  ...
  //根据ID获取 path_decision() 中的障碍物数据，并通过 Find 函数查找当前车辆面对的障碍物，并将其赋值给 blocking_obstacle 指针
  ...
  //获取当前车辆面对的障碍物在参考线上的起始位置，并将其赋值给 blocking_obstacle_s 变量。
  double blocking_obstacle_s =
      blocking_obstacle->PerceptionSLBoundary().start_s();
  //获取当前车辆在参考线上的结束位置，并将其赋值给 adc_end_s 变量。
  double adc_end_s = reference_line_info.AdcSlBoundary().end_s();
  ADEBUG << "Blocking obstacle is at s = " << blocking_obstacle_s;//输出日志信息，记录当前车辆面对的障碍物在参考线上的位置
  ADEBUG << "ADC is at s = " << adc_end_s;//输出日志信息，记录当前车辆在参考线上的位置。
  ADEBUG << "Destination is at s = "//输出日志信息，记录车辆的行驶目的地在参考线上的位置。
