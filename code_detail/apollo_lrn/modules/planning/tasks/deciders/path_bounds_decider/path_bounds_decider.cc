...
Status PathBoundsDecider::Process(
  ...
  PathBound fallback_path_bound; //这里边界都是对车辆中心而言
  Status ret =
      // 生成fallback_path_bound，无论什么场景都会生成
      // 首先计算当前位置到本车道两侧车道线的距离；
      // 然后判断是否借道，并根据借道方向叠加相邻车道的车道宽度
      // 带有adc的变量表示与自车相关的信息
      GenerateFallbackPathBound(*reference_line_info, &fallback_path_bound);//这里其实是根据道路边界，是否借道，借道方向，车辆宽度等更新车辆中心的l边界，还未考虑障碍物对边界的影响
  if (!ret.ok()) {
  ...
  //std::get用于获取元组中的元素， 元组: tuple<double, double, double>
  if (!fallback_path_bound.empty()) {
    CHECK_LE(adc_frenet_l_, std::get<2>(fallback_path_bound[0]));//fallback_path_bound包括了离散点的s,和上下边界。
    CHECK_GE(adc_frenet_l_, std::get<1>(fallback_path_bound[0]));
  }
  // Update the fallback path boundary into the reference_line_info.
  std::vector<std::pair<double, double>> fallback_path_bound_pair;//可行区域上下边界
  // 使用emplace_back，在容器内部进行构造
  ...
  candidate_path_boundaries.emplace_back(std::get<0>(fallback_path_bound[0]),//std::get<0>(fallback_path_bound[0]是s信息
                                         kPathBoundsDeciderResolution,
                                         fallback_path_bound_pair);// 将fallback_path_bound存入到candidate_path_boundaries
  candidate_path_boundaries.back().set_label("fallback");
 /*  依次判断是否处于pull_ over、 lane_ change、 regular; 当处于其中一个场景，计算对应的path_bound并返回结果;即以上3种场景，
  只会选种生成对应的根据场景生成另外-条path bound。 */
  ...
  if (plan_pull_over_path) {
    ...
    if (!ret.ok()) {
      ...
      candidate_path_boundaries.emplace_back(
          ...
          pull_over_path_bound_pair);// 将pull_over_path_bound存入到candidate_path_boundaries
      candidate_path_boundaries.back().set_label("regular/pullover");

      reference_line_info->SetCandidatePathBoundaries(//这里的reference_line_info是外面通过引用传递来的
  ...
  if (FLAGS_enable_smarter_lane_change &&
    ...
    candidate_path_boundaries.emplace_back(
        ...
        lanechange_path_bound_pair);// 将lanechange_path_bound存入到candidate_path_boundaries
  ...
  if (reference_line_info->is_path_lane_borrow()) {
    ...
    for (const auto& lane_borrow_direction :
      ...
      }//前面的stage判别过了，可向左，可向右借道，有时候两侧都可变道，都放进list
    }
  }
  ...
  for (const auto& lane_borrow_info : lane_borrow_info_list) {//Try every possible lane-borrow option
    ...
    candidate_path_boundaries.back().set_blocking_obstacle_id(
        blocking_obstacle_id);//设置当前被阻碍障碍物的ID
  }
  ...
  reference_line_info->SetCandidatePathBoundaries(
      std::move(candidate_path_boundaries)); 
  //填充好candidate_path_boundaries（fallback一定有，其他三个lane-change、pull-over、regular选择一个激活）后，将candidate_path_boundaries给到reference_line_info的candidate_path_boundaries_。
...
Status PathBoundsDecider::GenerateRegularPathBound(
  ...
  //常规情况下，就没有前面两种情况下对path_bound做特殊约束。
...
Status PathBoundsDecider::GenerateLaneChangePathBound(
  ...
  // Append some extra path bound points to avoid zero-length path data.
  // 附加一些额外的路径边界点以避免零长度的路径数据，kNumExtraTailBoundPoint内的边界点数据不能为空。
  int counter = 0;
  while (!blocking_obstacle_id.empty() &&
         path_bound->size() < temp_path_bound.size() &&
         counter < kNumExtraTailBoundPoint) {
...
Status PathBoundsDecider::GeneratePullOverPathBound(
  ...
  // 1. Initialize the path boundaries to be an indefinitely large area.
  if (!InitPathBoundary(reference_line_info, path_bound)) {//同样需要初始化path_bound
    const std::string msg = "Failed to initialize path boundaries.";
    AERROR << msg;
    return Status(ErrorCode::PLANNING_ERROR, msg);
  }
  ...
  // 2. Update boundary by lane boundary for pull_over 根据道路边界更新path_bound
  UpdatePullOverBoundaryByLaneBoundary(reference_line_info, path_bound);
  // PathBoundsDebugString(*path_bound);

  // 3. Fine-tune the boundary based on static obstacles 根据静态障碍物边界更新path_bound和中心线
  PathBound temp_path_bound = *path_bound;
  std::string blocking_obstacle_id;
  if (!GetBoundaryFromStaticObstacles(reference_line_info.path_decision(),
                                      path_bound, &blocking_obstacle_id)) {
  ...
  // If haven't found a pull-over position, search for one.如果没有发现靠边停车位置，则搜索一下
  if (curr_idx < 0) {
  ...
  // Trim path-bound properly //根据kNumExtraTailBoundPoint，修剪掉多余的边界
...
bool PathBoundsDecider::InitPathBoundary(
  ...
  //根据kPathBoundsDeciderResolution=0.5m ，每0.5米取一个控制点，直到预测位置(可通过预测时间或者预测距离确定)
  //初始化边界点，为每个边界点进行初始化，左侧边界为超级大，右侧边界为超级小
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    if (CheckLaneBoundaryType(reference_line_info, curr_s, lane_borrow_info)) {
      ...
      if (lane_borrow_info == LaneBorrowInfo::LEFT_BORROW) {
        ...
        if (reference_line_info.GetNeighborLaneInfo(
                ReferenceLineInfo::LaneType::LeftForward, curr_s,
                &neighbor_lane_id, &curr_neighbor_lane_width)) {
          ADEBUG << "Borrow left forward neighbor lane.";//左侧前进
        } else if (reference_line_info.GetNeighborLaneInfo(
                       ReferenceLineInfo::LaneType::LeftReverse, curr_s,
                       &neighbor_lane_id, &curr_neighbor_lane_width)) {
          borrowing_reverse_lane = true;
          ADEBUG << "Borrow left reverse neighbor lane.";//左侧掉头
        } else {
          ADEBUG << "There is no left neighbor lane.";
        }
      } else if (lane_borrow_info == LaneBorrowInfo::RIGHT_BORROW) {
        ...
        if (reference_line_info.GetNeighborLaneInfo(
                ReferenceLineInfo::LaneType::RightForward, curr_s,
                &neighbor_lane_id, &curr_neighbor_lane_width)) {
          ADEBUG << "Borrow right forward neighbor lane.";//右侧前进
        } else if (reference_line_info.GetNeighborLaneInfo(
                       ReferenceLineInfo::LaneType::RightReverse, curr_s,
                       &neighbor_lane_id, &curr_neighbor_lane_width)) {
          borrowing_reverse_lane = true;
          ADEBUG << "Borrow right reverse neighbor lane.";//右侧掉头
        } else {
          ADEBUG << "There is no right neighbor lane.";
        }
      }
    ...
    //    and ADC's velocity.根据车道宽度、ADC的位置和ADC的速度计算适当的边界
    static constexpr double kMaxLateralAccelerations = 1.5;//横向最大瞬时加速度
    double offset_to_map = 0.0;
    reference_line.GetOffsetToMap(curr_s, &offset_to_map);

    double ADC_speed_buffer = (adc_frenet_ld_ > 0 ? 1.0 : -1.0) *
                              adc_frenet_ld_ * adc_frenet_ld_ /
                              kMaxLateralAccelerations / 2.0;//计算最大横向瞬时位移
    ...
    if (config_.path_bounds_decider_config()
      ...
      // bounds.
      double curr_left_bound_adc =
          std::fmax(adc_l_to_lane_center_,//adc_l_to_lane_center_是当前车辆中心l位置
      ...
      double curr_right_bound_adc =
          std::fmin(adc_l_to_lane_center_,//adc_l_to_lane_center_是当前车辆中心l位置
                    adc_l_to_lane_center_ + ADC_speed_buffer) -
          GetBufferBetweenADCCenterAndEdge() - ADC_buffer;
      curr_right_bound =
    ...
    // 4. Update the boundary.覆盖填充更新
    //curr_left_bound和curr_right_bound为刚计算得到的左右边界，path_bound为之前获得的边界容器，path_bound指针传入会被更新
    //在此之前,path_bound仅仅被初始化过，所有控制点的上下边界为double类型的最大或者最小值，需要根据刚计算得到的左右边界对其进行覆盖填充更新
    if (!UpdatePathBoundaryWithBuffer(i, curr_left_bound, curr_right_bound,
...
void PathBoundsDecider::GetBoundaryFromLaneChangeForbiddenZone(
  ...
  if (lane_change_status->exist_lane_change_start_position()) {//如果存在换道开始的位置
    common::SLPoint point_sl;
    ...
    lane_change_start_s = FLAGS_lane_change_prepare_length + adc_frenet_s_;//当前s位置加准备长度
...
bool PathBoundsDecider::GetBoundaryFromStaticObstacles(
  ...
  double center_line = adc_frenet_l_;//车辆的中心线位置
  size_t obs_idx = 0;//当前处理的障碍物下标
  int path_blocked_idx = -1;//阻挡路径的障碍物的路径下标
  std::multiset<double, std::greater<double>> right_bounds;//车辆右侧的障碍物位置，std::greater<double>为降序参数
  //std::numeric_limits<double>::lowest()表示表示 double 类型的最小值。由于 std::multiset
  // 是一个有序容器，因此插入最小值可以确保容器中始终存在一个最小值，这对于后续的处理非常重要。
  right_bounds.insert(std::numeric_limits<double>::lowest());
  std::multiset<double> left_bounds;//默认升序
  ...
  std::unordered_map<std::string, bool> obs_id_to_direction;//每个障碍物的通过方向
  // Maps obstacle ID's to the decision of whether side-pass on this obstacle
  // is allowed. If allowed, then true; otherwise, false.
  std::unordered_map<std::string, bool> obs_id_to_sidepass_decision;//每个障碍物是否可以进行侧向通行

  // Step through every path point.
  for (size_t i = 1; i < path_boundaries->size(); ++i) {//从第二个路径点开始，因为第一个路径点是起点
    double curr_s = std::get<0>((*path_boundaries)[i]);//获取当前路径点的 s 值
    // Check and see if there is any obstacle change:
    //如果障碍物还有，且当前路径点的 s 值大于等于障碍物的位置，则检查是否有新的障碍物进入或者已有的障碍物离开了路径。
    if (obs_idx < sorted_obstacles.size() &&
        std::get<1>(sorted_obstacles[obs_idx]) < curr_s) {
      //如果当前路径点的 s 值大于等于障碍物的位置，则从障碍物列表中查找所有位置小于当前路径点 s 值的障碍物。
      while (obs_idx < sorted_obstacles.size() &&
             std::get<1>(sorted_obstacles[obs_idx]) < curr_s) {
        const auto& curr_obstacle = sorted_obstacles[obs_idx];// curr_obstacle类型为(is_start_s, s, l_min, l_max, obstacle_id)
        ...
        if (std::get<0>(curr_obstacle) == 1) {//如果当前障碍物是新进入的障碍物。
          ...
          if (curr_obstacle_l_min + curr_obstacle_l_max < center_line * 2) {//如果当前障碍物在车辆中心线的右侧，则表示需要从左侧绕过障碍物。
            // Obstacle is to the right of center-line, should pass from left.
            obs_id_to_direction[curr_obstacle_id] = true;
            right_bounds.insert(curr_obstacle_l_max);//注意，这是没问题的，curr_obstacle_l_max是障碍物左边的位置，这条边是可行路径的右边界
          } else {
            // Obstacle is to the left of center-line, should pass from right.
            obs_id_to_direction[curr_obstacle_id] = false;
            left_bounds.insert(curr_obstacle_l_min);//注意，这是没问题的，curr_obstacle_l_min是障碍物右边的位置，这条边是可行路径的左边界
          }

          //注意：每个控制点处，right_bounds和left_bounds只有其中一个会被insert


          if (!UpdatePathBoundaryAndCenterLineWithBuffer(//根据障碍物约束和Buffer中的路径边界，更新路径边界和车辆中心线的位置
                  i, *left_bounds.begin(), *right_bounds.begin(),//由于left_bounds是一个multiset类型对象，*left_bounds.begin()表示最小值，*right_bounds.begin()表示最大值
        ...
        if (std::get<1>((*path_boundaries)[i]) >
            std::get<2>((*path_boundaries)[i])) {//可行域小于0，本车道完全堵住
          ADEBUG << "Path is blocked at s = " << curr_s;
          path_blocked_idx = static_cast<int>(i);
          if (!obs_id_to_direction.empty()) {
            *blocking_obstacle_id = obs_id_to_direction.begin()->first;//记录被阻碍的障碍物ID。
          }
          break;
        } else {
      ...
      // 由于++obs_idx，当前障碍物飞到控制点前面，不会加入if，这段时间内的控制点的边界和上一个控制点保持一致，
      // 直到控制点的s超过当前障碍物的s，才会根据当前障碍物来更新边界。
      std::get<1>((*path_boundaries)[i]) =
...
bool PathBoundsDecider::UpdatePathBoundaryWithBuffer(
  ...
  //左正，右边负，这里右边界更新，需要取相对左边一点的才行
  double new_l_min =
      std::fmax(std::get<1>((*path_boundaries)[idx]),
                right_bound + right_adc_buffer_coeff * //right_adc_buffer_coeffs是安全因子
                                  GetBufferBetweenADCCenterAndEdge());//GetBufferBetweenADCCenterAndEdge()计算的是车辆宽度的一半
  //需要考虑车宽，因为计算边界对车辆中心而言的

  // Update the left bound (l_max):
  //左正，右边负，这里左边界更新，需要取相对右边一点的才行
  ...
  std::get<1>((*path_boundaries)[idx]) = new_l_min;//右边边界覆盖更新
  std::get<2>((*path_boundaries)[idx]) = new_l_max;//左边边界覆盖更新
  return true;
}
...
bool PathBoundsDecider::UpdatePathBoundaryAndCenterLineWithBuffer(
  ...
  //left_bound，right_bound为左右障碍物的边界，path_boundaries已有的边界比较，同时考虑车宽和安全距离，获取安全边界
