...
Status STBoundaryMapper::ComputeSTBoundary(PathDecision* path_decision) const {
  // Sanity checks.
  //这段代码实现了一个ST图边界计算器（STBoundaryMapper），用于计算路径上的障碍物在ST图上的边界，以便规划器做出更好的路径规划决策。
  ...
  for (const auto* ptr_obstacle_item : path_decision->obstacles().Items()) {//遍历每一个障碍物
    Obstacle* ptr_obstacle = path_decision->Find(ptr_obstacle_item->Id());//根据ID定位障碍物
    ACHECK(ptr_obstacle != nullptr);

    // If no longitudinal decision has been made, then plot it onto ST-graph.
    //如果该障碍物没有纵向决策，则计算其在ST图上的边界。
    if (!ptr_obstacle->HasLongitudinalDecision()) {
      ComputeSTBoundary(ptr_obstacle);
      continue;
    }

    // If there is a longitudinal decision, then fine-tune boundary.如果该障碍物有纵向决策
    const auto& decision = ptr_obstacle->LongitudinalDecision();
    if (decision.has_stop()) {//如果决策是停车
      ...
      reference_line_.XYToSL(decision.stop().stop_point(), &stop_sl_point);//将停车点获取，并转化到sl坐标系，填充到stop_sl_point地址
      const double stop_s = stop_sl_point.s();

      if (stop_s < min_stop_s) { // 第一个需要停车决策的障碍物，才会进行更新，因为已经停下来了
        stop_obstacle = ptr_obstacle;
        min_stop_s = stop_s;//更新离车辆最近的停车的s值
        stop_decision = decision;
      }
    } else if (decision.has_follow() || decision.has_overtake() ||
               decision.has_yield()) {//如果决策是跟随、超车或让行
    ...
    } else if (!decision.has_ignore()) {//如果是忽略决障碍物的决策类型
...
bool STBoundaryMapper::GetOverlapBoundaryPoints(
  ...
  // Draw the given obstacle on the ST-graph.绘制障碍物点在st图上
  const auto& trajectory = obstacle.Trajectory();
  if (trajectory.trajectory_point().empty()) {//如果没有预测轨迹，将其视为一个静态的障碍物。
    ...
    for (const auto& curr_point_on_path : path_points) {//遍历每个路径点
      if (curr_point_on_path.s() > planning_max_distance_) {
        break;
      }

      const Box2d& obs_box = obstacle.PerceptionBoundingBox();
      if (CheckOverlap(curr_point_on_path, obs_box, l_buffer)) {//根据障碍物几何信息(也就是最前和最后的s)检测，如果检测出有重叠部分
...
void STBoundaryMapper::ComputeSTBoundaryWithDecision(
  ...
  if (FLAGS_use_st_drivable_boundary &&
      obstacle->is_path_st_boundary_initialized()) {//如果已经初始化了
  ...
  //根据车道线和障碍物的几何信息，计算障碍物在ST图中的上下边界点。且只有与所规划路径有重叠部分的障碍物才会处理
    if (!GetOverlapBoundaryPoints(path_data_.discretized_path(), *obstacle,
                                  &upper_points, &lower_points)) {
