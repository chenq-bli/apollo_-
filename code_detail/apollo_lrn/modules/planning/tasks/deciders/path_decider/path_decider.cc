...
Status PathDecider::Process(const ReferenceLineInfo *reference_line_info,
                            ...
                            PathDecision *const path_decision) {//path_decision是指针传入的，直接对path_decision_操作
 //上一个任务已经选出了不同约束类型路径下最优的路径，这个任务根据选出的路径对障碍物做出决策
...
bool PathDecider::MakeStaticObstacleDecision(
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {//根据障碍物与最优路径的相对位置信息，生成车辆与障碍物之间的行驶决策
