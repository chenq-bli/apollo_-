...
Status OnLanePlanning::Init(const PlanningConfig& config) {//在初始化中，就确定使用哪个planner
  config_ = config;//这个类中也有config_，存放config
  ...
  planner_dispatcher_->Init();//后面会使用到planner选择，因此初始化中也对选择器进行初始化，里面会注册所有的planner
  //注册的planner主要有RTK、PUBLIC_ROAD、LATTICE、NAVI
  ...
  reference_line_provider_->Start();//启动子线程，以20HZ的频率运行，用于计算planning中最重要的数据结构reference_line

  // dispatch planner
  planner_ = planner_dispatcher_->DispatchPlanner(config_, injector_);//根据参数文件选择一个规划器
  if (!planner_) {
    return Status(
  ...
  return planner_->Init(config_);//对选择的规划器进行初始化
}
...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  frame_.reset(new Frame(sequence_num, local_view_, planning_start_point,
                         vehicle_state, reference_line_provider_.get()));//这里构造的时候没有更新reference_line_info_，在下面
  ...
  std::list<ReferenceLine> reference_lines;//创建一个临时reference_lines对象
  std::list<hdmap::RouteSegments> segments;
  if (!reference_line_provider_->GetReferenceLines(&reference_lines,
                                                   &segments)) {//从reference_line_provider_获取reference_lines
  ...
  //每次执行RunOnce都会进行参考线的更新，会调用frame_->Init来将reference_line_provider_获取的最新参考线信息更新到frame_的reference_line_info_中。
  auto status = frame_->Init(//关键来了，这里进行frame_的init，里面会将reference_line_provider_获取的reference_lines给到frame_的reference_line_info_，进行更新。
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  Status status = injector_->vehicle_state()->Update(
      *local_view_.localizatinjector_ion_estimate, *local_view_.chassis);
  ...
  if (!status.ok() || !util::IsVehicleStateValid(vehicle_state)) {//车辆状态没有准备好
    ...
    GenerateStopTrajectory(ptr_trajectory_pb);//生成停止轨迹，并直接返回
    return;
  }
  ...
  if (util::IsDifferentRouting(last_routing_, *local_view_.routing)) {//如果路由变换
    last_routing_ = *local_view_.routing;
    ADEBUG << "last_routing_:" << last_routing_.ShortDebugString();
    injector_->history()->Clear();
    injector_->planning_context()->mutable_planning_status()->Clear();//清理
    reference_line_provider_->UpdateRoutingResponse(*local_view_.routing);//更新
    planner_->Init(config_);//从新初始化规划器
  }
  ...
  if (failed_to_update_reference_line) {//如果参考线更新失败
  ...
  //初始化，获得frame_,这里面对frme进行构造，构造时传入reference_line_provider_，可将reference_line_provider_对象中的reference_lines_内容放到frame_的reference_line_info_中，后面方便获取
...
//路径规划入口
  status = Plan(start_timestamp, stitching_trajectory, ptr_trajectory_pb);//ptr_trajectory_pb指针传入，保存规划轨迹
...
Status OnLanePlanning::Plan(
  ...
  //注意：Plan（）中ptr_trajectory_pb还没有被所规划的填充轨迹点，frame_中的reference_line_info 才存放了，后面会利用frame_对ptr_trajectory_pb的trajectory_point_指针所指向的地址中
  
  //stitching_trajectory.back()：路径规划的起点。在 Apollo 中，路径规划通常是基于历史轨迹进行的，stitching_trajectory 是一个历史轨迹的容器，其中 back() 函数返回的是最后一个轨迹点，即路径规划的起点。
  //frame_.get()：规划的框架（Frame）。在 Apollo 中，框架是指规划器计算路径时所需要的上下文信息，例如当前车辆状态、障碍物信息等等。frame_ 是一个指向当前框架的智能指针，调用 get() 函数可以获取指针所指向的框架对象。
  //ptr_trajectory_pb：规划结果的指针。这段代码是 Apollo 中规划器（Planner）模块的核心代码之一，其作用是进行路径规划，并将规划结果保存到一个 protobuf 格式的消息中。
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {//如果在open_space
    frame_->mutable_open_space_info()->sync_debug_instance();
    const auto& publishable_trajectory =
        frame_->open_space_info().publishable_trajectory_data().first;//获取可发布的开放空间轨迹。在开放空间路径规划中，规划器通常会计算出一条最优路径，并将其保存到开放空间信息中。
    const auto& publishable_trajectory_gear =
        frame_->open_space_info().publishable_trajectory_data().second;//second 成员变量表示轨迹对应的档位信息。
    publishable_trajectory.PopulateTrajectoryProtobuf(ptr_trajectory_pb);//将可发布的轨迹数据填充到 protobuf 格式的消息中
    ptr_trajectory_pb->set_gear(publishable_trajectory_gear);//设置轨迹对应的档位信息
    ...
    //在众多ReferenceLineInfo寻找最小代价的ReferenceLineInfo，通过遍历寻找最小代价值所对应的ReferenceLineInfo。
    ...
    const auto& best_ref_path = best_ref_info->path_data().discretized_path();//获取最优参考路径的路径点
    std::copy(best_ref_path.begin() + 1, best_ref_path.end(),
              std::back_inserter(current_frame_planned_path));//将best_ref_path中元素复制到current_frame_planned_path，用于在容器的末尾创建一个插入迭代器（Insert Iterator），并返回该迭代器，这样复制就不会覆盖已有数据了
    ...
    last_publishable_trajectory_.reset(new PublishableTrajectory(
        current_time_stamp, best_ref_info->trajectory()));//释放原来智能指针，关联新指针，从best_ref_info成员discretized_trajectory_中获取所规划的轨迹，利用assign(trajectory.trajectory_point().begin(),trajectory.trajectory_point().end())函数拷贝到PublishableTrajectory对象中
    ...
    last_publishable_trajectory_->PopulateTrajectoryProtobuf(ptr_trajectory_pb);//这一步很重要，收尾，将所规划的轨迹点数据拷贝到ptr_trajectory_pb指针所指向的对象地址中，上一层函数会进行轨迹点的发布
    //Protobuf类型的
