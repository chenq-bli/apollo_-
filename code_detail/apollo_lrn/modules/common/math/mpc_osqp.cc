...
//需要先再纸上写出矩阵Ac，再构造相同列数的columns二维向量存放矩阵Ac中各列的非零元素的位置和值信息，遍历columns的列和行将Ac中各列的非零元素的位置和值信息存放到A_indices和A_data，再后面可根据A_data，A_indices，A_indptr把data->A中非0元素修改从而获得data->A
  ...
  columns.resize(num_param_ + 1);//这是列，和Ac矩阵的列相同，确实是这样的，验证过了
  int value_index = 0;
...
OSQPData *MpcOsqp::Data() {
  ...
  size_t kernel_dim = state_dim_ * (horizon_ + 1) + control_dim_ * horizon_;//根据预测逻辑，前一时刻的状态和控制量预测下一时刻的状态量，因此控制纬度比状态纬度小1
  size_t num_affine_constraint =
      2 * state_dim_ * (horizon_ + 1) + control_dim_ * horizon_;//这里的2是因为状态量具有等式约束和不等式约束，因此是2倍
  if (data == nullptr) {
    return nullptr;
  } else {
    ...
    data->P =
        csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
                   CopyData(P_indices), CopyData(P_indptr));//根据P矩阵非零元素的位置信息和值信息构造data->P，它是一个压缩稀疏列（CSC）格式的矩阵
    ADEBUG << "Get P matrix";
