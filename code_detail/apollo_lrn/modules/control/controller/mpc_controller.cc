...
namespace apollo
{
  namespace control
  {
    ...
    bool MPCController::LoadControlConf(const ControlConf *control_conf)
    {
      ...
      //定位control.conf.proto 中的optional apollo.control.MPCControllerConf mpc_controller_conf = 34;
      //上面有import "modules/control/proto/mpc_controller_conf.proto";
      //mpc_controller_conf.proto中有optional double cr = 3; optional double max_lateral_acceleration = 14; 等
      ...
      lf_ = wheelbase_ * (1.0 - mass_front / mass_);//质心到前
      lr_ = wheelbase_ * (1.0 - mass_rear / mass_);//质心到后
    ...
    {
      if (!LoadControlConf(control_conf))//从control_conf中获取参数，给到MPCController
      {
    ...
    Status MPCController::ComputeControlCommand(
        ...
        ControlCommand *cmd)
    {
      // 先构造，再用move将其左值，返回其右值（内容）给到trajectory_analyzer_，拷贝一份期望轨迹
      trajectory_analyzer_ =//轨迹分析器包括了一些根据距离最短或者时间最短求解匹配点的方法，用这个类来实现
          std::move(TrajectoryAnalyzer(planning_published_trajectory));

      SimpleMPCDebug *debug = cmd->mutable_debug()->mutable_simple_mpc_debug();
      debug->Clear();
      // 计算纵向误差等，station_error，speed_error，acceleration_error等放在debug指向的内存中
      ComputeLongitudinalErrors(&trajectory_analyzer_, debug);

      // Update state，计算横向偏，横向偏差变化率差，航向偏差和航向偏差变化率，并更新状态矩阵
      UpdateState(debug);

      // 将矩阵matrix_a_和matrix_c_更新，并离散化获得matrix_ad_，和matrix_cd_
      UpdateMatrix(debug);
      ...
      // Add gain scheduler for higher speed steering 算法开增益调度表，开的话权重与车速相关，默认不开启
      ...
      else
      {
        matrix_q_updated_ = matrix_q_;
        matrix_r_updated_ = matrix_r_;
        steer_angle_feedforwardterm_updated_ = steer_angle_feedforwardterm_; // 转向角前馈更新
      }
      // 将权重矩阵填充到debug
      ...
      Matrix control_matrix = Matrix::Zero(controls_, 1);    // 单个控制量
      std::vector<Matrix> control(horizon_, control_matrix); // 创建控制序列，horizon_为时域(个)，初始化horizon_个control_matrix
      ...
      // reference_state为0，因为状态都是偏差和偏差的倒数，因此目标就是使得他们为0
      Matrix reference_state = Matrix::Zero(basic_state_size_, 1); // 单个状态量，包括了lateral_error, lateral_error_rate, heading_error, heading_error_rate，station_error, station_error_rate
      std::vector<Matrix> reference(horizon_, reference_state);    // 创建参考状态序列，初始化horizon_个reference_state
      // 设置控制量约束
      ...
      // 初始化控制量，后面将控制序列的第一个控制量填充在这里
      std::vector<double> control_cmd(controls_, 0);

      // 构造apollo::common::math空间里的MpcOsqp求解器，这里是构造
      // 传入参数有：离散的A矩阵 matrix_ad_，离散的B矩阵 matrix_bd_，权重矩阵Q matrix_q_updated_，权重矩阵R matrix_r_updated_，车辆当前状态 matrix_state_
      // 控制量约束 lower_bound和upper_bound，状态量约束 lower_state_bound和upper_state_bound，参考状态 reference_state，最大迭代次数 mpc_max_iteration_，
      // 预测时域 horizon_，精度 mpc_eps_
      // 需要首先构建二次规划问题
      // reference_state为0，因为状态都是偏差和偏差的倒数，因此目标就是使得他们为0
      //不同的模型的这些参数代入，都可套用，其实已经结束，内部是进行预测，对目标函数构造QP问题，调用QP求解器
      //这里的matrix_q_updated_维度是六维方阵，是base，是单个状态量内每个状态的权重情况，在Q求解中也有Q矩阵，那个是这里Q矩阵的扩充矩阵，改扩展矩阵的对角有预测时域个这里的Q。同理lower_bound这些也一样需要扩充。
      apollo::common::math::MpcOsqp mpc_osqp(
          ...
          mpc_eps_);                     // 构造MpcOsqp的一个对象，还不是求解
      if (!mpc_osqp.Solve(&control_cmd)) // mpc_osqp.Solve(&control_cmd)求解，并填充control_cmd
      //解出的control_cmd[0]为前轮转角和control_cmd[1]为加速度，后面需要转化为方向盘转角比例和油门开度
      {
    ...
    void MPCController::UpdateMatrix(SimpleMPCDebug *debug)
    {
      ...
      // 利用欧拉法或者双线性变换法将其离散化获得matrix_ad_
        ...
        const TrajectoryAnalyzer &trajectory_analyzer, SimpleMPCDebug *debug)
    {
      ...
      debug->set_lateral_error(cos_matched_theta * dy - sin_matched_theta * dx);//横向偏差，计算参考apollo学习文档 lQR控制器最后。

      // matched_theta = matched_point.path_point().theta();
      debug->set_ref_heading(matched_point.path_point().theta());
      const double delta_theta =
          common::math::NormalizeAngle(theta - debug->ref_heading());//由于角度值在数学上是一个周期性的值，即每 2π 为一个周期，因此在进行角度差值计算时，需要对结果进行归一化，以确保其范围在 [-π, π] 内
        ...
        const TrajectoryAnalyzer *trajectory_analyzer, SimpleMPCDebug *debug)
    {
      ...
      // 根据当前坐标寻找期望轨迹上最近点作为匹配点，根据当前点和匹配点之间关系，将当前点转化到轨迹的坐标系下(sl坐标系)，这个不是参考点，而是真实点
      // 寻找期望轨迹上最近点
      ...
      // s_matched, s_dot_matched, d_matched, d_dot_matched为将当前点转化到轨迹的坐标系下的s,l,ds,dl
      ...
      // debug更新位置参考station_reference、位置反馈station_feedback、位置误差station_error、速度参考speed_reference、
      // 速度反馈speed_feedback、速度误差speed_error、加速度参考acceleration_reference、加速度反馈acceleration_feedback、加速度误差acceleration_error
