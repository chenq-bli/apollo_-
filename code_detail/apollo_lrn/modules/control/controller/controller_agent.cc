...
/*  new 构造一个子类MPCController，利用父类指针Controller*指向之类对象(ProductCreator是AbstractProduct的指针类型，
 而AbstractProduct就是父类Controller，这里利用了模板知识)，然后将id和父类指针插入到map类型的MapContainer里面，后面
 会根据id遍历将父类指针们放到controller_list_里面。 */
          break;
    ...
    Status ControllerAgent::InitializeConf(const ControlConf *control_conf)
    {
      ...
      // control_conf->active_controllers()中要么是一个 MPC controller，要么是Lat + Lon controllers两个控制器
      for (auto controller_type : control_conf_->active_controllers())//control_conf_->active_controllers()是激活的控制器id，上面RegisterControllers中注册的是3个控制器都有，这里进行选择使用
      {
        //static_cast是C++风格的类型转换，static_cast<int>(x)将x转换为整形，相当于C风格的（int）x
        auto controller = controller_factory_.CreateObject(//controller_factory_所对应的类是模板类，可能其他模块也使用到了
            static_cast<ControlConf::ControllerType>(controller_type)); // 利用producers_成员寻找到指向所注册的控制器的指针，并创建智能指针指向，并返回
        if (controller)
        {                                                       // 如果智能指针关联的指针不为空
          controller_list_.emplace_back(std::move(controller)); // 取消智能指针与原指针的关联，并将原指针emplace_back()进入controller_list_，原指针为Controller*,是MPCController和LatController和LonController的父类指针。
        }
        else
        {
                                 ...
                                 const ControlConf *control_conf)
    {
      ...
      ACHECK(InitializeConf(control_conf).ok()) << "Failed to initialize config."; // 将指向需要执行的控制器的父类智能指针加入到controller_list_中
      for (auto &controller : controller_list_)
      {
        ...
        if (!controller->Init(injector, control_conf_).ok())//利用多态对个控制器进行初始化
    ...
    //ADCTrajectory在/modules/common_msgs/planning_msgs/planning.proto中定义,用了protobuf消息类型
    //Chassis在/modules/common_msgs/chassis_msgs/chassis.proto中定义,用了protobuf消息类型
    {
