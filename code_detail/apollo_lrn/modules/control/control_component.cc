...
namespace apollo
{
  namespace control
  {
    ...
    using apollo::planning::ADCTrajectory;//using + 类 后使用类时不要前面的作用域了 
    ...
    bool ControlComponent::Init()
    {
      ...
      ACHECK(
          cyber::common::GetProtoFromFile(FLAGS_control_conf_file, &control_conf_)) // 加载参数文件到control_conf_成员
          //通过包含control_gflags.h头文件，可以直接访问FLAGS_control_conf_file全局变量。
          /*在control_gflags.cc中
      ...
      // initial controller agent when not using control submodules 对controller_agent_进行初始化
      ...
      planning_reader_config.pending_queue_size = FLAGS_planning_pending_queue_size; // FLAGS_planning_pending_queue_size参数文件获取

      trajectory_reader_ =
          node_->CreateReader<ADCTrajectory>(planning_reader_config, nullptr); // 创建轨迹话题订阅器
      ...
      { // 创建控制命令发布器
        control_cmd_writer_ =
            node_->CreateWriter<ControlCommand>(FLAGS_control_command_topic);//FLAGS_control_command_topic为话题名称 参数
        ...
        const std::shared_ptr<ADCTrajectory> &trajectory)
    {
      ...
      std::lock_guard<std::mutex> lock(mutex_); // 复制数据的时候加线程锁，防止在复制数据期间其他线程又读取新数据。
        ...
        ControlCommand *control_command)
    {
      ...
      {
        Status status_ts = CheckTimestamp(local_view_); // 继续检查时间戳
        if (!status_ts.ok())
        {
      ...
      if (!estop_)
      {
        ...
        // controller agent 基于传入的LocalizationEstimate、Chassis和ADCTrajectory参数来填充ControlCommand结构
    ...
    bool ControlComponent::Proc()
    {
      ...
      const auto &chassis_msg = chassis_reader_->GetLatestObserved(); // 从Chassis的发布渠道读取最新的消息
      ...
      OnChassis(chassis_msg); // 将读取到的消息拷贝到latest_chassis_成员

      trajectory_reader_->Observe();
      const auto &trajectory_msg = trajectory_reader_->GetLatestObserved(); // 从trajectory的发布渠道读取最新的消息
      if (trajectory_msg == nullptr)
      { // 如果指向空
        AERROR << "planning msg is not ready!";
        return false;
      }
      OnPlanning(trajectory_msg); // 将读取到的消息拷贝到ControlComponent类的latest_trajectory_成员
      ...
      // 调用Ｗriter的Write方法将填充好的ControlCommand发送出去：
